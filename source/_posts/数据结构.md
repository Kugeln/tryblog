...
- **原理**：
  - Bellman-Ford算法通过不断“松弛”所有的边，来逐步找到从源点到其他所有点的最短路径。其关键思想是：从源点出发，每次放松所有的边，最多需要松弛|V| - 1次（其中V是图中节点的数量）。
  - 它可以处理**负权边**，并且能检测**负权回路**。
- **操作过程**：
  1. 初始化：将源点到自己设置为0，其他所有点到源点的距离设置为无穷大。
  2. 对图中的每一条边，检查是否可以通过该边更新最短路径。如果可以更新，则松弛该边。
  3. 重复步骤2，执行|V| - 1次。
  4. 如果在|V| - 1次之后，仍然能够松弛某条边，则说明图中存在负权回路。
- **时间复杂度**：O(V * E)，其中V是顶点数，E是边数。
- **适用场景**：适用于**带负权边**的图（但是不能包含负权回路）。

2. **Dijkstra算法**

- **原理**：
  - Dijkstra算法基于贪心思想，逐步从源点扩展最短路径，始终选择当前未访问的、距离源点最近的节点，更新它的相邻节点的最短路径。
  - 它不适用于**负权边**（因为负权边可能导致最短路径被错误地更新）。
- **操作过程**：
  1. 初始化：将源点的距离设置为0，其他点的距离设置为无穷大。
  2. 将源点加入优先队列，队列中存放当前已知的最短路径最小的节点。
  3. 每次从队列中弹出当前距离最小的节点，并更新其邻接节点的最短路径。
  4. 重复步骤3，直到所有节点都被访问。
- **时间复杂度**：
  - 使用**线性查找**时，时间复杂度为O(V^2)。
  - 使用**二叉堆**时，时间复杂度为O((V + E) * logV)。
  - 使用**斐波那契堆**时，时间复杂度为O(E + V * logV)。
- **适用场景**：适用于**非负权边**的图。

3. **SPFA算法**（Shortest Path Faster Algorithm）

- **原理**：
  - SPFA算法是对Bellman-Ford算法的优化，通常使用**队列**（而不是松弛所有边）来进行最短路径的更新，从而提高了效率。
  - 它与Bellman-Ford类似，每次放松一条边，但SPFA使用队列管理节点，使得算法只更新需要更新的节点，从而避免了不必要的操作。
  - SPFA通常表现得比Bellman-Ford更高效，但仍然无法处理负权回路（如果存在）。
- **操作过程**：
  1. 初始化：源点到自己为0，其他点到源点的距离为无穷大，将源点放入队列。
  2. 从队列中取出一个点，遍历它的邻接点，进行松弛操作。
  3. 如果邻接点的距离发生变化，将它加入队列。
  4. 重复步骤2和3，直到队列为空。
- **时间复杂度**：O(V + E * logV)（依赖于队列的实现，通常情况下，队列的实现是一个简单的线性队列或者优先队列）。
- **适用场景**：适用于**非负权边**的图，并且比Dijkstra在某些图上效率更高。
